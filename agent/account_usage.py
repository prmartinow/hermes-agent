...
def _fetch_gemini_account_usage(
    *,
    ...
    from hermes_cli.auth import (
        ...
        _normalize_gemini_account_id,
        resolve_session_last_used_account,
        get_account_alias,
        _read_gemini_account_tokens,
    )

    acc_identifier = account
    if not acc_identifier and session_id:
        try:
            acc_identifier = resolve_session_last_used_account(session_id)
        except Exception:
            acc_identifier = None

    if not acc_identifier and provider:
        m = re.match(r"^gemini(?:-oauth)?-([1-5])$", str(provider).strip().lower())
        if m:
            acc_identifier = int(m.group(1))

    acc_idx = 1
    if isinstance(acc_identifier, int) and 1 <= acc_identifier <= 5:
        acc_idx = acc_identifier
    elif acc_identifier:
        raw = str(acc_identifier).strip()
        raw_lower = raw.lower()
        m = re.search(r"gemini(?:-oauth)?-([1-5])", raw_lower)
        if m:
            try:
                acc_idx = max(1, min(5, int(m.group(1))))
            except ValueError:
                acc_idx = 1
        else:
            found = False
            for idx in range(1, 6):
                try:
                    tokens = _read_gemini_account_tokens(idx)
                    if tokens:
                        email = str(tokens.get("email") or "").strip()
                        alias = get_account_alias(email) if email else ""
                        if raw_lower in (str(idx), email.lower(), alias.lower(), f"gemini-{idx}", f"gemini-oauth-{idx}"):
                            acc_idx = idx
                            found = True
                            break
                except Exception:
                    continue
            if not found:
                acc_idx = _normalize_gemini_account_id(acc_identifier)
    ...
    if not status.get("logged_in"):
        return AccountUsageSnapshot(
            ...
            source="gemini_oauth",
            fetched_at=_utc_now(),
            title="Active Account Quota",
            unavailable_reason=f"Account {acc_idx} is not logged in",
        )

    alias = status.get("alias") or get_account_alias(status.get("email", "")) or f"Account {acc_idx}"
    quota = status.get("quota") or {}

    windows: list[AccountUsageWindow] = []

    # 1. Gemini Models (5h & Weekly)
    g_5h_pct = quota.get("gemini_5h_percent")
    g_5h_reset = quota.get("gemini_5h_reset")
    g_5h_cd = quota.get("gemini_5h_countdown")
    if g_5h_pct is not None:
        used_pct = round(max(0.0, min(100.0, 100.0 - float(g_5h_pct))), 2)
        dt = _parse_dt(g_5h_reset)
        detail = f"resets in {g_5h_cd}" if g_5h_cd else None
        windows.append(
            AccountUsageWindow(
        ...
        windows.append(
            AccountUsageWindow(
                label="Gemini 5h Limit",
                detail=f"resets in {g_5h_cd}",
            )
        )

    g_w_pct = quota.get("gemini_weekly_percent")
    g_w_reset = quota.get("gemini_weekly_reset")
    g_w_cd = quota.get("gemini_weekly_countdown")
    if g_w_pct is not None:
        used_pct = round(max(0.0, min(100.0, 100.0 - float(g_w_pct))), 2)
        dt = _parse_dt(g_w_reset)
        detail = f"resets in {g_w_cd}" if g_w_cd else None
        windows.append(
            AccountUsageWindow(
        ...
        windows.append(
            AccountUsageWindow(
                label="Gemini Weekly Limit",
                detail=f"resets in {g_w_cd}",
            )
        )

    # 2. Claude & 3P Partner Models (5h & Weekly)
    c_5h_pct = quota.get("claude_5h_percent")
    c_5h_reset = quota.get("claude_5h_reset")
    c_5h_cd = quota.get("claude_5h_countdown")
    if c_5h_pct is not None:
        used_pct = round(max(0.0, min(100.0, 100.0 - float(c_5h_pct))), 2)
        dt = _parse_dt(c_5h_reset)
        detail = f"resets in {c_5h_cd}" if c_5h_cd else None
        windows.append(
            AccountUsageWindow(
                label="Claude/3P 5h Limit",
                used_percent=used_pct,
                reset_at=dt,
                detail=detail,
            )
        ...
        windows.append(
            AccountUsageWindow(
                label="Claude/3P 5h Limit",
                detail=f"resets in {c_5h_cd}",
            )
        )

    c_w_pct = quota.get("claude_weekly_percent")
    c_w_reset = quota.get("claude_weekly_reset")
    c_w_cd = quota.get("claude_weekly_countdown")
    if c_w_pct is not None:
        used_pct = round(max(0.0, min(100.0, 100.0 - float(c_w_pct))), 2)
        dt = _parse_dt(c_w_reset)
        detail = f"resets in {c_w_cd}" if c_w_cd else None
        windows.append(
            AccountUsageWindow(
                label="Claude/3P Weekly Limit",
                used_percent=used_pct,
                reset_at=dt,
                detail=detail,
            )
        ...
        windows.append(
            AccountUsageWindow(
                label="Claude/3P Weekly Limit",
                detail=f"resets in {c_w_cd}",
            )
        )

    details = (f"Active Account: {alias}",)
...
def fetch_account_usage(
    ...
    *,
    ...
    if not norm or norm in {"auto", "custom"}:
        return None
    if norm in {"gemini-oauth", "gemini_oauth", "gemini"} or re.match(r"^gemini(?:-oauth)?-[1-5]$", norm):
        try:
            ...
            return None
    fetcher = _USAGE_FETCHERS.get(norm)
    try:
