...
def is_standard_key_auth_error(
    status: int, error_message: str, reason: str = "", *, api_key: str = "", url: str = "",
) -> bool:
...
def _translate_tool_result_to_gemini(
    ...
    name = str((tool_name_by_call_id or {}).get(tool_call_id) or message.get("tool_name") or message.get("name") or tool_call_id or "tool")
    raw_content = message.get("content")
    content = json.dumps(raw_content, ensure_ascii=False) if isinstance(raw_content, dict) else _coerce_content_to_text(raw_content)
    try:
...
def _translate_tools_to_gemini(tools: Any, *, json_schema: bool = False) -> List[Dict[str, Any]]:
    translated = build_gemini_tools(tools)
    if json_schema:
        # Keep grounding and deterministic declaration ordering from the shared
        # builder, but preserve full schemas on API versions that support them.
        parameters_by_name = {}
        for tool in tools if isinstance(tools, list) else []:
            if not isinstance(tool, dict):
                continue
            fn = tool.get("function", tool)
            if isinstance(fn, dict) and isinstance(fn.get("name"), str) and isinstance(fn.get("parameters"), dict):
                parameters_by_name[fn["name"]] = fn["parameters"]
        for tool in translated:
            for declaration in tool.get("functionDeclarations", []):
                parameters = parameters_by_name.get(declaration["name"])
                if parameters is not None:
                    declaration.pop("parameters", None)
                    declaration["parametersJsonSchema"] = prepare_gemini_tool_parameters(parameters)
    return translated
...
def gemini_http_error(
    ...
    if is_standard_key_auth_error(status, err_message or body_text, reason, api_key=api_key, url=req_url):
        ...
        **kwargs: Any,
        ...
        if tools:
            if isinstance(tools, list) and tools and isinstance(tools[0], dict) and "functionDeclarations" in tools[0]:
                ...
                gemini_tools = _translate_tools_to_gemini(
                    tools, json_schema=gemini_accepts_parameters_json_schema(self.base_url)
                )
                if gemini_tools:
