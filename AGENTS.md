...
### Automated Daily Upstream Sync Workflow
Hermes Agent on this host is synchronized with upstream `NousResearch/hermes-agent` daily via a user systemd timer:
- **Script**: `scripts/sync_upstream_daily.sh`
- **Systemd Service**: `~/.config/systemd/user/hermes-upstream-sync.service`
- **Systemd Timer**: `~/.config/systemd/user/hermes-upstream-sync.timer` (runs daily at 04:00)
- **Workflow**:
  1. Stashes any active local modifications.
  2. Fetches `upstream` and `origin`.
  3. Fast-forwards `main` to `upstream/main` and pushes to `origin/main`.
  4. Merges `main` into `gemini`, runs tests, and pushes to `origin/gemini`.
  5. Merges `gemini` into `local`, runs tests, pushes to `origin/local`, and restarts `hermes-dashboard.service`.
  6. Restores original branch and stash, logging all actions to `~/.hermes/logs/upstream_sync.log`.
