...
Probes hermes-agent-serving (:9119) and Host OS
"""
...
h_dir = os.environ.get("HERMES_HOME", str(Path.home() / ".hermes"))
...
gemini_dir = Path.home() / ".gemini" if os.path.exists(str(Path.home() / ".gemini")) else Path(f"{h_dir}/.gemini")
...
memory_home = Path(os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes"))
traces_path = Path(os.environ.get("AGENT_MEMORY_TRACES_PATH") or str(memory_home / "agent-memory" / "traces"))
memory_dir = traces_path.parent
mem_stat = memory_dir.stat() if memory_dir.exists() else None
mem_rw = os.access(memory_dir, os.W_OK) if memory_dir.exists() else False
...
from pathlib import Path
db_stat = os.stat(str(Path.home() / ".hermes/state.db")) if os.path.exists(str(Path.home() / ".hermes/state.db")) else None
skills_stat = os.stat(str(Path.home() / ".hermes/skills")) if os.path.exists(str(Path.home() / ".hermes/skills")) else None
gemini_stat = os.stat(str(Path.home() / ".gemini")) if os.path.exists(str(Path.home() / ".gemini")) else None
memory_home = Path(os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes"))
traces_path = Path(os.environ.get("AGENT_MEMORY_TRACES_PATH") or str(memory_home / "agent-memory" / "traces"))
memory_dir = traces_path.parent
mem_stat = memory_dir.stat() if memory_dir.exists() else None
...
lease_file = Path(str(Path.home() / ".hermes/runtime/quota_refresher.lease"))
lease_data = json.loads(lease_file.read_text()) if lease_file.exists() else None
...
res = {
    ...
    "db_inode": db_stat.st_ino if db_stat else None,
    "skills_inode": skills_stat.st_ino if skills_stat else None,
    "skills_count": len([d for d in Path(str(Path.home() / ".hermes/skills")).iterdir() if d.is_dir() and not d.name.startswith(".")]) if Path(str(Path.home() / ".hermes/skills")).exists() else 0,
    "gemini_inode": gemini_stat.st_ino if gemini_stat else None,
    "mem_inode": mem_stat.st_ino if mem_stat else None,
    "mem_rw": mem_rw,
    "agy_bin": agy_bin,
