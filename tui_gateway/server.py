...
def _teardown_session(session: dict | None, *, end_reason: str = "tui_close") -> None:
    ...
    try:
        if sid := session.get("_sid"):
            _stop_external_stream_tailer(sid)
    except Exception:
        pass
    try:
        agent = session.get("agent")
...
def _backend_id_for_this_process() -> str:
    ...
    return f"{profile}@{host}:{os.getpid()}:{nonce}"
...
def write_json(obj: dict) -> bool:
    ...
    if obj.get("method") == "event":
        ...
        if sid and (sess := _sessions.get(sid)) is not None:
            from tui_gateway.event_replay import _stamp_event

            _stamp_event(obj)

            viewers = sess.get("viewers")
            if isinstance(viewers, dict) and viewers:
                delivered = False
                dead_transports = []
                for vt in list(viewers.keys()):
                    try:
                        if vt.write(obj):
                            delivered = True
                    except Exception:
                        dead_transports.append(vt)
                for dt in dead_transports:
                    viewers.pop(dt, None)
                if delivered:
                    return True
...
def _emit_settled_session_info(sid: str, session: dict, agent) -> None:
    ...
    info_payload = _session_info(agent, session)
    try:
        gemini_acc = info_payload.get("gemini_account")
        db = getattr(agent, "_session_db", None) or getattr(agent, "session_db", None) or (session or {}).get("db")
        if gemini_acc and db and hasattr(db, "update_session_meta"):
            ...
            if session_key:
                ...
                if sess:
                    ...
                    if not isinstance(cfg, dict):
                        cfg = {}
                    if cfg.get("gemini_account") != gemini_acc:
                        cfg["gemini_account"] = gemini_acc
                        db.update_session_meta(session_key, json.dumps(cfg), model=getattr(agent, "model", None))
    except Exception:
        pass

    _emit("session.info", sid, info_payload)
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    * A terminal session (``hermes`` / ``hermes --tui`` / CLI) is started from a
    ...
    row_provider = str(model_config.get("provider") or override.get("provider") or "").strip().lower()
    try:
        db.create_session(
            key,
            source=_session_source(session),
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if model:
        ...
        gemini_account = str(model_config.get("gemini_account") or "").strip()
        if gemini_account:
            overrides["model_override"]["gemini_account"] = gemini_account
    if provider:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    active_provider = str(pending_provider or mirror.get("provider", getattr(agent, "provider", "")) or "")
    gemini_account = ""
    if active_provider in {"gemini-oauth", "gemini_oauth"}:
        try:
                ...
                if entry_id:
                    for e in pool.entries():
                        if e.id == entry_id:
                            ...
                            break
                if not entry:
                    sess_cfg = (session or {}).get("model_config") or {}
                    if isinstance(sess_cfg, str):
                        try:
                            import json
                            sess_cfg = json.loads(sess_cfg)
                        except Exception:
                            sess_cfg = {}
                    if not isinstance(sess_cfg, dict):
                        sess_cfg = {}
                    pinned_acc = sess_cfg.get("gemini_account")
                    if not pinned_acc:
                        db = getattr(agent, "_session_db", None)
                        session_key = str((session or {}).get("session_key") or getattr(agent, "session_id", "") or "")
                        if db and session_key and hasattr(db, "get_session"):
                            sess_row = db.get_session(session_key)
                            if sess_row:
                                raw_cfg = sess_row.get("model_config")
                                ...
                                if isinstance(raw_cfg, dict):
                                    pinned_acc = raw_cfg.get("gemini_account")
                    if pinned_acc:
                        from hermes_cli.auth import get_account_alias
                        target_norm = str(pinned_acc).strip().lower()
                        for e in pool.entries():
                            e_label = str(e.label or "").strip().lower()
                            e_id = str(e.id or "").strip().lower()
                            e_alias = str(get_account_alias(e.label or e.id) or "").strip().lower()
                            if target_norm in (e_label, e_id, e_alias):
                                entry = e
                                break
                if not entry:
                    entry = pool.current() or pool.peek()
                if entry:
                    raw_acc = str(entry.label or entry.id or "").strip()
                    from hermes_cli.auth import get_account_alias

                    gemini_account = get_account_alias(raw_acc)
        except Exception:
            pass
...
# ── External-session live stream tailer ────────────────────────────────
_external_stream_watchers: dict[str, Any] = {}
_external_stream_watchers_lock = threading.Lock()


def _start_external_stream_tailer(session_key: str, client_sid: str, profile_home: Optional[str] = None) -> None:
    """Start tailing an external CLI session's live event log for client_sid."""
    session_key = str(session_key or "").strip()
    client_sid = str(client_sid or "").strip()
    if not session_key or not client_sid:
        return

    with _external_stream_watchers_lock:
        existing = _external_stream_watchers.get(client_sid)
        if existing is not None and getattr(existing, "_thread", None) is not None and existing._thread.is_alive():
            # Already actively tailing for this session
            return

        watcher_ref = [None]

        def _on_watcher_idle():
            with _external_stream_watchers_lock:
                if _external_stream_watchers.get(client_sid) is watcher_ref[0]:
                    _external_stream_watchers.pop(client_sid, None)
            session = _sessions.get(client_sid)
            if session is not None and session.get("agent") is None:
                session["running"] = False
                session["status"] = "idle"
                try:
                    _emit("session.info", client_sid, _session_info(session.get("agent"), session))
                except Exception:
                    pass

        try:
            from agent.live_session_stream import LiveSessionStreamWatcher

            watcher = LiveSessionStreamWatcher(
                session_id=session_key,
                client_sid=client_sid,
                emit_fn=_emit,
                profile_home=profile_home,
                on_idle=_on_watcher_idle,
            )
            watcher_ref[0] = watcher
            _external_stream_watchers[client_sid] = watcher
            watcher.start()
        except Exception as exc:
            logger.debug("Failed to start LiveSessionStreamWatcher for %s: %s", session_key, exc)


def _stop_external_stream_tailer(client_sid: str) -> None:
    """Stop live tailer for client_sid."""
    with _external_stream_watchers_lock:
        watcher = _external_stream_watchers.pop(client_sid, None)
...
def _make_agent(
    ...
    # Trigger non-blocking MCP discovery in background thread if not already running.
    # NEVER block the interactive critical path with wait_for_mcp_discovery().
    try:
        from hermes_cli.mcp_startup import start_background_mcp_discovery

        start_background_mcp_discovery(logger=logger, thread_name=f"mcp-bg-{sid}")
    except Exception:
        pass
    ...
    preferred_session_acc = None
    if isinstance(model_override, dict) and model_override.get("gemini_account"):
        preferred_session_acc = model_override.get("gemini_account")
    elif isinstance(model_override, dict) and model_override.get("preferred_account"):
        preferred_session_acc = model_override.get("preferred_account")
    if not preferred_session_acc and session_db and (session_id or key):
        try:
            from hermes_cli.auth import resolve_session_last_used_account
            preferred_session_acc = resolve_session_last_used_account(session_id or key, session_db)
        except Exception:
            pass
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if preferred_session_acc:
            resolve_kwargs["preferred_account"] = preferred_session_acc
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "viewers": {current_transport() or _stdio_transport: now} if (current_transport() or _stdio_transport) is not None else {},
        }
...
def _lazy_resume_info(
    ...
    *,
    ...
    if gemini_account:
        info["gemini_account"] = gemini_account
    return info
...
def _session_live_title(session: dict, key: str) -> str:
    title = str(session.get("title") or session.get("pending_title") or "").strip()
    if title:
        return title
    try:
        with _session_db(session) as db:
...
def _serialize_billing_state(state) -> dict:
    ...
    if state.payment_method is not None:
        alias2 = state.payment_method
        # Each kind sends only its own fields. Emitting every key with nulls
        # would contradict the shared type — a client checking `'brand' in alias2`
        ...
        if alias2.kind == "card":
            payment_method = {
                ...
                "brand": alias2.brand,
                "last4": alias2.last4,
                "wallet": alias2.wallet,
                "resolved_via": alias2.resolved_via,
            }
        elif alias2.kind == "link":
            payment_method = {
                "kind": "link",
                "email": alias2.email,
                "resolved_via": alias2.resolved_via,
    ...
    *,
    ...
    if display_kind != "hidden":
        _emit(
            "turn.start",
            sid,
            {"user_message": text if isinstance(text, str) else str(text or "")},
        )
    _emit("message.start", sid)
