...
const parseMouseTracking = (v?: string): MouseTrackingMode | null => {
  const raw = (v ?? '').trim().toLowerCase()
  if (!raw) return null
  if (raw === 'buttons' || raw === 'all' || raw === 'wheel' || raw === 'off') {
    return raw as MouseTrackingMode
  }
  if (truthy(raw)) return 'buttons'
  if (falsy(raw)) return 'off'
  return null
}

const mouseTrackingOverride = parseMouseTracking(process.env.HERMES_TUI_MOUSE_TRACKING)
