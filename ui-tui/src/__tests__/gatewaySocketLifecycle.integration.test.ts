...
function waitForSocketResponse(ws: WsServerClient, expectedId: string, timeoutMs = 5000): Promise<string> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out after ${timeoutMs}ms waiting for socket response ${expectedId}`))
    }, timeoutMs)
    ...
    const onMessage = (data: any) => {
      ...
      try {
        ...
        cleanup()
        reject(new Error(`Failed to parse frame: ${raw}`))
        return
      }
      ...
      if (frame.method === "session.events.since") {
        ws.send(
          JSON.stringify({
            ...
            id: frame.id,
            result: {
              epoch: frame.params?.epoch || "epoch-1",
              events: [],
              latest_seq: frame.params?.last_seen ?? 0,
              truncated: false
            }
          })
        )
      ...
      cleanup()
      reject(new Error(`Unexpected frame received while waiting for ${expectedId}: ${raw}`))
    }
...
describe("GatewayClient real WebSocket lifecycle integration regression", () => {
  ...
  it("delivers ready, session completion, and server request exactly once after abnormal drop and reconnect without remount/drain", async () => {
    ...
    const s2ResponsePromise = waitForSocketResponse(s2, "srq-approve-1")
    ...
    const s3ResponsePromise = waitForSocketResponse(s3, "srq-clarify-2")
