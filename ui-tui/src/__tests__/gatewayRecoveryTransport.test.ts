...
describe('Gateway Recovery: WebSocket Transport Loss vs Process Exit', () => {
  ...
  it('preserves live turn/busy and reconnect target with truthful transport copy on socket 1006 loss', () => {
    ...
    // Assert: GatewayClient reconnect owns backoff; start() is not called and process budget is not consumed
    expect(startMock).not.toHaveBeenCalled()
    expect(recoveryAtRef.current).toHaveLength(0)
  })
  ...
  it('bounds restart attempts and gave-up notification on repeated process exits', () => {
    const sysMock = vi.fn()
    const pushActivityMock = vi.spyOn(turnController, 'pushActivity')
    const startMock = vi.fn()
    ...
    // 1st close: live sid present, clears sid, initiates restart
    exitHandler(1, { code: 1, source: 'process' })
    expect(getUiState().sid).toBeNull()
    expect(recoverSidRef.current).toBe('session-repeated-789')
    expect(recoveryAtRef.current).toHaveLength(1)
    expect(startMock).toHaveBeenCalledTimes(1)

    // 2nd close while sid is null (restart attempt failed before ready)
    exitHandler(1, { code: 1, source: 'process' })
    expect(getUiState().sid).toBeNull()
    expect(recoverSidRef.current).toBe('session-repeated-789')
    ...
    expect(pushActivityMock).toHaveBeenCalledWith(BACKEND_GAVE_UP_ACTIVITY, 'error')
    expect(sysMock).toHaveBeenCalledWith(expect.stringContaining('Hermes stopped (exit code 1)'))
  ...
  it('retains reconnect target across repeated socket loss without calling gw.start, resetting turn, or consuming restart budget', () => {
    const sysMock = vi.fn()
    const pushActivityMock = vi.spyOn(turnController, 'pushActivity')
    const resetSpy = vi.spyOn(turnController, 'reset')
    const startMock = vi.fn()
    ...
    patchUiState({ busy: true, sid: 'session-repeated-socket', status: 'working…' })
    ...
    // 1st close: live sid present, clears sid, sets reconnect target, preserves busy, no start/reset, unchanged attempts
    const plan1 = exitHandler(1006, { code: 1006, source: 'websocket' })
    expect(plan1.recover).toBe(true)
    expect(plan1.sid).toBe('session-repeated-socket')
    expect(plan1.attempts).toHaveLength(0)
    expect(getUiState().sid).toBeNull()
    expect(getUiState().busy).toBe(true)
    expect(getUiState().status).toBe('reconnecting…')
    expect(recoverSidRef.current).toBe('session-repeated-socket')
    expect(recoveryAtRef.current).toHaveLength(0)
    expect(startMock).not.toHaveBeenCalled()
    expect(resetSpy).not.toHaveBeenCalled()
    expect(pushActivityMock).toHaveBeenCalledWith(TRANSPORT_RECONNECTING_ACTIVITY, 'warn')
    ...
    // 2nd close while sid is null: retains reconnect target, busy preserved, 0 start, 0 reset, unchanged attempts
    const plan2 = exitHandler(1006, { code: 1006, source: 'websocket' })
    expect(plan2.recover).toBe(true)
    expect(plan2.sid).toBe('session-repeated-socket')
    expect(plan2.attempts).toHaveLength(0)
    expect(getUiState().sid).toBeNull()
    expect(getUiState().busy).toBe(true)
    expect(recoverSidRef.current).toBe('session-repeated-socket')
    expect(recoveryAtRef.current).toHaveLength(0)
    ...
    // 3rd close while sid is null
    const plan3 = exitHandler(1006, { code: 1006, source: 'websocket' })
    expect(plan3.recover).toBe(true)
    expect(recoverSidRef.current).toBe('session-repeated-socket')
    expect(recoveryAtRef.current).toHaveLength(0)
    expect(startMock).not.toHaveBeenCalled()

    // 4th close while sid is null: does NOT exhaust budget (process budget untouched)
    const plan4 = exitHandler(1006, { code: 1006, source: 'websocket' })
    expect(plan4.recover).toBe(true)
    expect(recoverSidRef.current).toBe('session-repeated-socket')
    expect(getUiState().status).toBe('reconnecting…')
    expect(gaveUpRef.current).toBe(false)
    expect(recoveryAtRef.current).toHaveLength(0)
    expect(startMock).not.toHaveBeenCalled()
    expect(resetSpy).not.toHaveBeenCalled()
  })

  it('falls back to storedSid when sessionKey is absent', () => {
    const recoverSidRef = { current: null as string | null }
    const recoveryAtRef = { current: [] as number[] }
    const gaveUpRef = { current: false }
    const startMock = vi.fn()
    ...
    patchUiState({
      busy: true,
      sessionKey: null,
      storedSid: 'session-stored-fallback-789',
      sid: 'session-runtime-sid-ignored',
      status: 'working…'
    })

    const exitHandler = createGatewayExitHandler({
      gaveUpRef,
      gw: { isAttached: () => true, start: startMock, getLogTail: () => '' },
      recoverSidRef,
      recoveryAtRef,
      sys: vi.fn()
    })

    const plan = exitHandler(1006, { code: 1006, source: 'websocket' })

    expect(plan.sid).toBe('session-stored-fallback-789')
    expect(plan.recover).toBe(true)
    expect(recoverSidRef.current).toBe('session-stored-fallback-789')
  })

  it('prioritizes sessionKey over storedSid and sid', () => {
    const recoverSidRef = { current: null as string | null }
    const recoveryAtRef = { current: [] as number[] }
    const gaveUpRef = { current: false }
    const startMock = vi.fn()
    ...
    patchUiState({
      busy: true,
      sessionKey: 'session-primary-key-111',
      storedSid: 'session-stored-key-222',
      sid: 'session-runtime-sid-333',
      status: 'working…'
    })
    ...
    expect(plan.sid).toBe('session-primary-key-111')
    expect(plan.recover).toBe(true)
    expect(recoverSidRef.current).toBe('session-primary-key-111')
  })
