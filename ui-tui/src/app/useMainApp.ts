...
export function useMainApp(gw: GatewayClient) {
  ...
  useEffect(() => {
    ...
    const coalescer = createResizeCoalescer(async () => {
      const nextCols = stdout.columns ?? 80
      if (nextCols !== colsRef.current && (INLINE_MODE || DASHBOARD_TUI_MODE) && historyItemsRef.current.length > 5) {
        try {
          const stream = await formatCompleteHistory(historyItemsRef.current, nextCols)
          stdout.write('\x1b[2J\x1b[3J\x1b[H' + stream)
        } catch {
          /* ignore */
        }
      }
